...
final class SiteController extends Controller {
    ...
    public function services(): never { try{$services=$this->all('SELECT * FROM services WHERE is_active=1 ORDER BY display_order');}catch(\Throwable){$services=DemoContent::services();} $this->render('site/services',compact('services')); }
    public function service(string $slug): never { $faqs=[];$related=[];try{$service=$this->one('SELECT * FROM services WHERE slug=? AND is_active=1',[$slug]);if($service){$faqs=$this->all('SELECT question,answer FROM service_faqs WHERE service_id=? ORDER BY display_order',[$service['id']]);$related=$this->all('SELECT title,slug,summary FROM services WHERE id<>? AND is_active=1 ORDER BY display_order LIMIT 3',[$service['id']]);}}catch(\Throwable){$service=null;foreach(DemoContent::services() as $item)if($item['slug']===$slug)$service=$item;if($service){$faqs=DemoContent::faqs($service['title']);$related=array_slice(array_values(array_filter(DemoContent::services(),fn($item)=>$item['slug']!==$slug)),0,3);}} if(!$service) $this->render('errors/404',[],404);if(!$faqs)$faqs=DemoContent::faqs($service['title']);$this->render('site/service',compact('service','faqs','related')); }
    public function authors(): never { $q=trim($_GET['q']??'');try{$authors=$q!==''?$this->all('SELECT * FROM authors WHERE is_active=1 AND (name LIKE ? OR bio LIKE ?) ORDER BY name',["%$q%","%$q%"]):$this->all('SELECT * FROM authors WHERE is_active=1 ORDER BY name');}catch(\Throwable){$authors=array_values(array_filter(DemoContent::authors(),fn($author)=>$q===''||str_contains(strtolower($author['name'].' '.$author['bio']),strtolower($q))));} $this->render('site/authors',compact('authors','q')); }
    public function categories(): never {$categories=$this->all('SELECT c.*,COUNT(bc.book_id) book_count FROM categories c LEFT JOIN book_categories bc ON bc.category_id=c.id GROUP BY c.id ORDER BY c.name');$this->render('site/categories',compact('categories'));}
    public function author(string $slug): never { try{$author=$this->one('SELECT * FROM authors WHERE slug=? AND is_active=1',[$slug]);$books=$author?$this->all('SELECT * FROM books WHERE author_id=? AND is_active=1',[$author['id']]):[];}catch(\Throwable){$author=null;$books=[];foreach(DemoContent::authors() as $item)if($item['slug']===$slug)$author=$item;}if(!$author)$this->render('errors/404',[],404);$this->render('site/author',compact('author','books')); }
